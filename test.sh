#!/usr/bin/env bash
#
# civic-services-directory end-to-end smoke test.
#
...
image_id="civic-services-directory-web:smoke"
...
preflight_pass() {
  docker run --rm \
    ...
    --entrypoint /opt/civic-services-directory/preflight.sh "$image_id" \
...
"${COMPOSE[@]}" run --rm --no-deps \
  ...
  web sh -c "grep -qx 'Listen 8081' /etc/apache2/ports.conf \
    ...
    && PORT=8082 /usr/local/bin/civic-services-directory-entrypoint sh -c \"grep -qx 'Listen 8082' /etc/apache2/ports.conf && grep -q '<VirtualHost \\*:8082>' /etc/apache2/sites-available/000-default.conf\"" \
...
"${COMPOSE[@]}" run --rm --no-deps \
  ...
  web sh -c "a2dismod -f mpm_prefork mpm_event mpm_worker >/dev/null 2>&1 || true
    ...
    /usr/local/bin/civic-services-directory-entrypoint sh -c 'test -e /etc/apache2/mods-enabled/mpm_prefork.load && test ! -e /etc/apache2/mods-enabled/mpm_event.load && test ! -e /etc/apache2/mods-enabled/mpm_worker.load'" \
...
cid_a="$("${COMPOSE[@]}" run -d -e DRUPAL_SKIP_INSTALL=1 web php /opt/civic-services-directory/installer.php)"
cid_b="$("${COMPOSE[@]}" run -d -e DRUPAL_SKIP_INSTALL=1 web php /opt/civic-services-directory/installer.php)"
