# civic-services-directory: production-oriented Drupal 11 for Railway.
...
ENV PATH="/opt/drupal/vendor/bin:${PATH}" \
    RAILWAY_DIR="/opt/civic-services-directory" \
    DRUPAL_PERSISTENT_ROOT="/data"

# Production PHP settings (memory, uploads, opcache, errors hidden).
COPY drupal/php.ini /usr/local/etc/php/conf.d/30-civic-services-directory.ini

# Trust Railway's edge proxy so Drupal generates https:// URLs.
COPY drupal/apache-https.conf /etc/apache2/conf-available/civic-services-directory-https.conf
...
COPY docker/entrypoint.sh /usr/local/bin/civic-services-directory-entrypoint
RUN chmod +x /usr/local/bin/civic-services-directory-entrypoint \
    && mkdir -p /data/files \
...
ENTRYPOINT ["civic-services-directory-entrypoint"]
